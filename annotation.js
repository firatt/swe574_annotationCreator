class SegmentSelector {
  ...
  fromRange(range, inRange) {
    Object.defineProperty(this, 'tmp', { value: null, enumerable: false, writable: true });
    this.tmp = range;
  }
...
document.addEventListener("mousedown", function () {
  hexId = getRandomToken();
});

function setUserId(userId) {
  // get userid and set it inside annotation
  return userId;
}
...
class TextPositionSelector extends SegmentSelector {
  ...
  _textNodeIterator(inRange) {
    ...
    return document.createNodeIterator(commonAncestorContainer, NodeFilter.SHOW_TEXT, {
...
document.addEventListener("mouseup", function () {
  try {
    var selection = document.getSelection();
    var ab = new AnnotationBuilder().highlight(selection);
    
    jsonToBeSent = JSON.stringify(ab.result, false, '  ');
    // alert(jsonToBeSent);
    // console.log(jsonToBeSent);

    // Messaging 1
    chrome.runtime.sendMessage({action: "annoTransfer", obj: jsonToBeSent}, function(response) {
      // console.log(response.message);
    });

  } catch (error) {
    // console.log("Error during annotation creation: " + error);
  }
});